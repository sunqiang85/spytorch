...
def run(args, config):
    ...
    if not args.eval_only:
        train_dataset = dataset_module.dataset(config, split='train')
        train_loader = train_dataset.make_batch_loader()
    val_dataset = dataset_module.dataset(config, split='val')
    ...
    for epoch in range(start_epoch + 1, config.nb_epochs + 1):
        ...
        if result['accuracy_top1'] > best_accuracy_top1 and not args.eval_only:
    ...
    if start_epoch < config.nb_epochs:
